...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    // SHUTDOWN：关闭状态，不再接受新提交的任务，但却可以继续处理阻塞队列中已保存的任务。在线程池处于 RUNNING 状态时，
    // 调用 shutdown()方法会使线程池进入到该状态。（finalize() 方法在执行过程中也会调用shutdown()方法进入该状态）
    ...
    * */
    ...
    private final class Worker extends AbstractQueuedSynchronizer implements Runnable
    {
        ...
        // 所以一个Worker对象在启动的时候(调用start方法）会调用Worker类中的run方法。
        Worker(Runnable firstTask) {
            // 在构造方法中执行了setState(-1);，把state变量设置为-1，为什么这么做呢？
        ...
        // 所以setState(-1)将state设置为-1是为了禁止在执行任务前对线程进行中断。
