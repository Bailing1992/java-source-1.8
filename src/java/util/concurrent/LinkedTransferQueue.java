...
/**
 * An unbounded 无界的 {@link TransferQueue} based on linked nodes.
 * This queue orders elements FIFO (first-in-first-out) with respect to
 * any given producer.  The <em>head</em> of the queue is that
    ...
    private E xfer(E e, boolean haveData, int how, long nanos) {
        ...
        for (;;) {                            // restart on append race
            ...
            for (Node h = head, p = h; p != null;) { // find & match first node
                ...
                // (item != null) == isData 要么表示 p 是一个 put 操作, 要么表示 p 是一个还没匹配成功的 take 操作
            ...
            // 如果这个操作不是立刻就返回的类型
            if (how != NOW) {
                // 且是第一次进入这里// No matches available
                if (s == null)// 创建一个 node
